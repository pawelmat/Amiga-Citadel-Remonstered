...
start:		
		lea		CUSTOM,a0
		VBLANK
		move	#$7fff,INTENA(a0)		;INTENA
		move	#$7fff,INTREQ(a0)		;INTREQ
		move	#$00ff,ADKCON(a0)		;ADKONR
...
.cpTxt:	REPT 4
		...
		lea		sv_bomba,a1					; copy sinus to fast mem
		move.l	lc_F2_Bomba(pc),a2
		move	#(BOMB_SIZE/4)-1,d0
.cpBmb:	move.l	(a1)+,(a2)+
		dbf		d0,.cpBmb

		lea		sv_ChaosTab,a1					; copy chaos b, c, numbers, fonts, sinus to fast mem
		move.l	lc_F2_ChaosTab(pc),a2
		move	#(TABLES_SIZE/4),d0
.cpSin:	move.l	(a1)+,(a2)+
		dbf		d0,.cpSin
...
.cpRot:	REPT	8
		...
		lea		sv_MAP,a1					; copy MAP to fast mem
		move.l	lc_F2_Map(pc),a2
		move	#(MAP_FULL_SIZE/32)-1,d0
.cpMap:	REPT	8
		move.l	(a1)+,(a2)+
		ENDR
		dbf		d0,.cpMap
...
.ClrMap: move.l	#0,(a1)+
		...
		move.l	lc_F2_LevelData(pc),a1			; set player starting position based on the one in the new map
		move	16(a1),d0
		mulu	#1024,d0
		addi	#512,d0
		move	d0,sv_PosX
		...
		bmi.s	.ne1
		move	#999,d0
.ne1:	tst		lc_difficulty(a6)		;ustaw poziom trudnosci
...
.ne2:	neg	d0
		...
		tst		lc_difficulty(a6)		;ustaw poziom trudnosci
		beq.s	.sv_DIF			; 0 = hard (default)
	; easy settings
		move.l	lc_F2_EnemyData(pc),a1
		move.l	lc_F2_Map(pc),a2
		moveq	#62,d7
...
.sc_CopyCop:	move	(a1)+,(a2)+
		...
		lea		CUSTOM,a0
		move	#$7fff,DMACON(a0)
		move	#$8240,DMACON(a0)		;DMACON blitter on
		lea		sv_OldMouse,a1
		move	10(a0),(a1)			;remember starting mouse position

		waitblt
		VBLANK
		move	#$7fff,DMACON(a0)		;DMACON
		move	#$7fff,INTENA(a0)		;INTENA
		move	#$7fff,INTREQ(a0)
		move.l	lc_VbrBase(pc),a1
		...
		VBLANK
		move	#$83c0,DMACON(a0)		; DMACON dma and stuff
		move	#$c028,INTENA(a0)		; SET+INTEN, VERTB and PORTS
...
.waits:	move	lc_variables+lc_swapScreen(pc),d7
		...
		waitblt
		VBLANK
		move	#$7fff,INTENA(a0)			; INTENA - disable all interrupts
		move	#$7fff,INTREQ(a0)			; INTREQ - clearpending interrupts
...
.rm_DoUse:	SCROLL	53
		...
		move.l	lc_F2_SwitchData(pc),a2	;command tab
...
.oc_EnemyChk:	move	d1,d0
		...
		move.l	lc_F2_EnemyData(pc),a4		;EnemyTab
		andi	#$ff,d0
		lsl		#4,d0
		lea		(a4,d0.w),a4
...
oc_EnemyHit:	move	#3,-(sp)
		move.l	a6,-(sp)		; this is used in enemyHitServe
		bsr		EnemyHitServe
		move.l	(sp)+,a6
		bra.s	oc_CheckError
...
;		beq.s	eh_End
		...
		btst.b	#0,1(a4)
		beq.s	.me01
		move.b	4(a6),d2		;czy czuly na to?
		bra.s	.me02
.me01:		
		move.b	10(a6),d2
.me02:		
...
Move_Enemy:
		move.l	lc_F2_Map(pc),a1
		move.l	lc_F2_LevelData(pc),a6
		move.l	lc_F2_EnemyData(pc),a2		;EnemyTab
...
.m2:	move	d0,d2			; Dead enemy item drops
		bsr		GetRandom
		andi	#$ff,d0
		move	lc_variables+lc_difficulty(pc),d1
...
;-------------------------------------------------------------------
;Main draw walls & objects loop - scaning from map. No input.
DrawAll:
		movem.l	ALL,-(sp)
		lea		lc_variables(pc),a6
		move.l	lc_F2_Map(pc),a0
		move.l	lc_F2_Sinus(pc),a1
...
dr_checkEnemy:	
		...
		beq.s	dr_checkITEM
		move.l	lc_F2_EnemyData(pc),a5		;EnemyTab
		andi	#$ff,d4
		lsl		#4,d4
		move.b	1(a5,d4.w),d0
		andi	#$80,d0
...
.sca:	cmp		(a1,d0.w),d5
		...
		move	d0,d5
		addq	#2,d5
		cmpi	#(TEXTURE_CACHE_SIZE*2),d5
		bne.s	.ssav
		moveq	#0,d5
.ssav:	move	d5,lc_textureCacheNext(a6)	; move cache pointer to next slot
...
; .nc:	subq	#1,d2
		...
		bmi.s	.nc
		move	#maxWallHeigth*2,d2		; theoretically can be maxWallHeigth*2=700 but max practical value is 466
.nc:	subq	#1,d2

		; caculate Y interpolation table
		move.l	lc_F1_WallCode(pc),a0		; code tab
;		move.l	lc_F2_Htab(pc),a1			; slow Htab
;		move.l	lc_F1_WallCode(pc),a1		; transparent code tab
...
;		move.l	(a1,d5.w),4*700-4(a4)	; transparent wall code entry
		lea		(a1,d5.w),a2
		move.l	a2,4*700-4(a4)	; pointer to first (up) transparent wall code entry
...
sh_Right:
		...
		subq	#1,d6
		swap	d1				; merge words for longword based interpolation
		move	d3,d1
...
sh_BorOK1:
		...
		swap	d2
		move	#1,d2					; d0 (X) add value (+1 right, -1 left wall orientation)
		move	sh_EorWallDir1(pc),d3	; wall direction EOR value
...
sh_BorOK2:
		...
		swap	d2
		move	#-1,d2					; d0 (X) add value (+1 right, -1 left wall orientation)
		move	sh_EorWallDir2(pc),d3	; wall direction EOR value
		addq	#1,d0

sh_Cont:
		moveq	#0,d4

		tst		lc_c2pType(a6)				; 0 - blitter , 1 - cpu 
		beq		sh_MBlitter

		move.l	lc_zeroPtr(a6),d5			; zero pointer tab
		move	lc_screenPixX(a6),-(sp)		; screen width
; d0 - start X pixel offset on screen
; d1,d4 interpolation
; d2 - d0 add value (+1 or -1)
; d3 - EOR value for wall direction
; d5 - zero pointer
...
sh_MloopCPU:	
		...
		add		d2,d0			; running X index of screen collumns to use
		bmi.s	sh_Noline1
		cmp		(sp),d0
		bpl.s	sh_Noline1

		lea		(a0,d0.w),a2		;screen middle starting point in collumn
		...
		swap	d4
		eor		d3,d7					;fix wall direction - 0 for non-inverted, 63 for inverted wall, so effectively take pixels from the right side of the texture
...
sh_NoLine1:
		...
		lea		lc_variables(pc),a6
		move.l	d5,lc_zeroPtr(a6)
		swap	d2
		move	d2,lc_remainingCollumns(a6)
		lea		2(sp),sp
...
; (a6) - Y code drawing jump address
sh_MBlitter:
		move.l	lc_zeroPtr(a6),-(sp)		; zero pointer tab
		move	lc_screenPixX(a6),-(sp)		; screen width
...
sh_NoLine2:
		dbf		d6,sh_MloopBlitter

		lea		lc_variables(pc),a6
		move.l	2(sp),lc_zeroPtr(a6)
		swap	d2
		move	d2,lc_remainingCollumns(a6)
		lea		6(sp),sp
...
; save collumn for adding later because it has clear pixels
 sh_SaveZero2:
		move.l	2(sp),a6		; zero pointer tab
		move.l	a1,(a6)+		; wall addr
		move.l	a2,(a6)+		; screen addr
		move.l	[4*700]-4(a4),(a6)+	;manual drawing cell addr (htab)
		move	#0,(a6)+
...
.noRsX:
		...
		move	d3,-(sp)		; save Y for later use
...
sc_ColLoopFull:
		move	(sp)+,d5		;Y
		lsl		#2,d5
		move.l	lc_F1_WallCode(pc),a4		; fast code tab
		move.l	(a4,d5.w),a4				; wall drawing code for this collumn
		add		d5,d5
		move.l	lc_F2_WallCodeTrans(pc),a1	; transparent code tab
		lea		(a1,d5.w),a1
...
.sc_putOnStack:
		move.l	a1,(a6)+		;wall addr
		move.l	a2,(a6)+		;screen addr
		move.l	(sp),(a6)+		;draw transparent code addr (first pointer out of 2: up, down)
...
.sc_ColLoop:
		...
		move.l	a1,(a6)+		;wall addr
		move.l	a2,(a6)+		;screen addr
		move.l	2(sp),(a6)+		;draw transparent code addr (first pointer out of 2: up, down)
		move	6(sp),(a6)+		;up/down/norm coll. flag

.sc_NoLine:	
		dbf		d6,.sc_ColLoop
		lea		6(sp),sp	; take off AND mask and address
		bra.s	sc_PostLoop
...
sc_ColLoopMiddle:
		...
		move.l	lc_F2_Htab(pc),a1	;slow Htab
		lsl		#2,d5
		move.l	(a1,d5.w),a4		;Htab cell addr
...
ShowZeroWalls:
		...
		lea		lc_variables(pc),a6
		move.l	lc_zeroPtr(a6),a0		;Zero lines table - pointer at the end (last entry)
		moveq	#0,d6
		move	lc_screenPixX(a6),d6	; width in pixels
		move.l	lc_F2_ZeroTab(pc),a4	; zero tab start - for comparison only

shZ_ZeroLoop:
		cmpa.l	a4,a0
		beq.s	shZ_PixEnd		;if reached beg of table

		move	-(a0),d0		;what is the object pos: up, down, fullsize, mid, ...
		beq.s	shZ_FullHeigth	; zero - full heigth object
		cmpi	#$8000,d0
		beq		shZ_UpObject	; half (32) up
		cmpi	#$c000,d0
		beq		shZ_DownObject	; half (32) down
		cmpi	#$a000,d0
		beq		shZ_Down16		; quater (16) down
		cmpi	#$e000,d0
		beq		shZ_Up16		; quater (16) up
...
; Full heigth walls
shZ_FullHeigth:	
		move.l	-(a0),a5		;transparent draw code addr
		move.l	-(a0),a2		;screen addr
		move.l	-(a0),a1		;texture addr
		move.l	(a5),a3
		jsr		(a3)			; draw up half of collumn strip 
		move.l	4(a5),a3
		jsr		(a3)			; draw bottm half of collumn strip
 		bra.s	shZ_ZeroLoop
...
MAP_FULL_SIZE:				equ		$9a00
BOMB_SIZE:					equ		$15cc
TABLES_SIZE:				equ		($6c0+$800+$1ae+$300+$280+4)	; chaos b, c, numbers, fonts, sinus
TEXT_SIZE:					equ		$2000
...
lc_chunkyBufferEnd:		dc.l	0				; chunky buffer end address
lc_floorTexture:		dc.l	0				; floor texture address
lc_ceilingTexture:		dc.l	0				; ceiling texture address
...
lc_F2_WallCodeTrans:	dc.l	F2_WallCodeTrans,0
lc_F2_Map:				dc.l	F2_Map,0
lc_F2_LevelData:		dc.l	F2_LevelData,0
lc_F2_EnemyData:		dc.l	F2_EnemyData,0
lc_F2_SwitchData:		dc.l	F2_SwitchData,0
lc_F2_Bomba:			dc.l	F2_Bomba,0
lc_F2_ChaosTab:			dc.l	F2_ChaosTab,0
lc_F2_ChaosTac:			dc.l	F2_ChaosTac,0
lc_F2_Numbers:			dc.l	F2_Numbers,0
lc_F2_Fonts:			dc.l	F2_Fonts,0
lc_F2_Sinus:			dc.l	F2_Sinus,0
lc_F2_TextOffsets:		dc.l	F2_TextOffsets,0
lc_F2_Text:				dc.l	F2_Text,0
...
;-----------------------------------------------
		...
		move.l	lc_floorTexture(a6),a1			;floor texture addr
		move.l	lc_ceilingTexture(a6),a2			;ceiling texture addr
		move.l	lc_chunkyBufferEnd(a6),a3			;SVGA tab floor addr - starts on last row of the screen
...
.flc_D3:	
		...
		add		a5,d6
		addx.b	d2,d0			;inc X
		move	lc_variables+lc_chunkyWidth4_1(pc),d5	;X counter/4 -1 (48 for size 5)
...
c2p_Init:
		...
		bsr		c2p_Blitter_noStretch_Init		; initialise blitter C2P queue
...
.waitb:	tst		lc_blitC2PPos(a6)
		bne.s	.waitb

		;copy buffer from fast to chip
		move.l	lc_ChunkyBuffer(pc),a2
		move.l	lc_ChunkyBufChip(a6),a1			; Blitter C2P uses chunky buf in chip
		move.l	lc_variables+lc_screenBytes(pc),d2
		move.l	d2,d1
		lsr.l	#6,d2
 		subq	#1,d2
.flp1:	rept	16
		move.l	(a2)+,(a1)+		; fill in 64-byte chunks
		endr
		dbf		d2,.flp1
		andi	#63,d1
		lsr		#2,d1
...
.waits:	tst		lc_swapScreen(a6)
		...
		move	#$8040,DMACON(a0)		; blitter DMA on
		waitblt
...
; Initialise blitter C2P queue
c2p_Blitter_noStretch_Init:
		move.l	lc_variables+lc_ChunkyBufChip(pc),a6
...
.waits:	move	lc_variables+lc_swapScreen(pc),d7
		...
		sub.l	a1,a1				; screen start offset from 0
		move.l	a1,(a4)+			; screen start address
		lea		-2(a1,d0.w),a3		; screen end offset (pointer to last word)
...
sv_Bits:						; inner loop repeated for every 1..8 bits
		...
		move	d0,(a4)+		; bltcon0
		move	#0,(a4)+		; bltcon 1
		move	d5,(a4)+		; C dat (AND mask to select the right on screen bit)
		move.l	a6,(a4)+		; A addr (chunky buffer - WIDTH (e.g. 24) or bytes for bit X)
		move.l	a1,(a4)+		; B addr (screen previous data) / D addr
		move	d2,(a4)+		; blt size
		bra.s	sv_NextBit
...
sv_BitMin:	
		...
		move	#2,(a4)+		; bltcon 1
		move	d5,(a4)+		; C dat (AND mask to select the right on screen bit)
		move.l	a2,(a4)+		; A addr (chunky buffer - WIDTH (e.g. 24) or bytes for bit X)
		move.l	a3,(a4)+		; B addr (screen previous data) / D addr
		move	d2,(a4)+		; blt size
sv_NextBit:
...
; In: A0 - CUSTOM, a6 - lc_variables(pc)
blitC2PSteps:
		move	lc_blitC2PPos(a6),d0
		cmpi	#16*5*8,d0
		bne.s	.go
		clr		lc_blitC2PPos(a6)				; clear pos when all done
		bra.s	.exit
.go:	addi	#16,lc_blitC2PPos(a6)

		move.l	lc_F2_BlitC2PTab(pc),a1
		move.l	(a1)+,d2		; screen addr
		move	(a1)+,d1
		move	d1,BLTBMOD(a0)		;B mod
		move	d1,BLTDMOD(a0)		;D mod
		move	(a1)+,BLTAMOD(a0)	;A mod
		move.l	#-1,BLTAFWM(a0)		;WLWmasks
		lea		(a1,d0.w),a1

		move.l	(a1)+,BLTCON0(a0)		;bltcon0, bltcon 1: BSH and direction
		move	(a1)+,BLTCDAT(a0)		;C dat (AND mask to select the right on screen bit)
		move.l	(a1)+,BLTAPTH(a0)		;A addr (chunky buffer - WIDTH (e.g. 24) or bytes for bit X)
		move.l	(a1)+,d0			; screen offset
		add.l	d2,d0				; add screen address
		move.l	d0,BLTBPTH(a0)			;B addr (screen previous data)
		move.l	d0,BLTDPTH(a0)			;D addr (screen destination data)
		move	(a1)+,BLTSIZE(a0)
.exit:
		rts
...
cc_d:	cmpi.b	#$b7,d0			;g - details on/off
		bne.w	cc_F1_F8
		addi	#1,lc_details(a6)
		beq.s	.cc_d2
		cmpi	#2,lc_details(a6)
		beq.s	.cc_d3
		cmpi	#3,lc_details(a6)
		beq.s	.cc_d4
		SCROLL	69
		bra		cc_NoKey
.cc_d3:	SCROLL	64
		bra		cc_NoKey
.cc_d4:	move	#0,lc_details(a6)
.cc_d2:	SCROLL	63
		bra		cc_NoKey
...
br_FindTeleport:
		move.l	lc_F2_SwitchData(pc),a2	;CommandTab
.rm_SeekPos:	
		move	(a2)+,d4
		cmpi	#-1,d4
		bne.s	.rm_SeekPos
...
;		lea		sv_Compas,a2
		...
		moveq	#26,d6
		waitblt
		move	#$0040,DMACON(a0)		;blitter off...
...
tc_DrawCardCnt:	
		...
		lea		sv_Cards,a1
		move.l	lc_F2_Numbers(pc),a2
		lea		[20*18](a2),a2		; small numbers
...
.c1:	move.l	d0,(a1)+
		...
		move.l	lc_F2_Numbers(pc),a1
		lea		[20*18](a1),a1	; small numbers
...
.fast:	bsr.s	StatusLedsShowCol
		...
		bsr.s	StatusLedsShowCol
		move	lc_variables+lc_c2pType(pc),d0		; 0 - blitter (red), 1 - cpu (green)
;		eori	#1,d0
		bsr.s	StatusLedsShowCol
		rts
...
; This version is for solid objects/walls. No input.
MakeCodeSolid:
		movem.l	ALL,-(sp)
...
.mc_nd:	move	#$1551,(a2)+		;move	(a1),y(a2)
		...
		add		d3,d0				; d3 = chunky row width
		dbf		d6,.mc_nd
.mc_n1:	move	#$1559,(a2)+		;move	(a1)+,y(a2)
		move	d0,(a2)+
		add		d3,d0				; d3 = chunky row width
		rts
...
;-------------------------------------------------------------------
; Make code for copying columns to bit-table... Kane/SCT, 09.02.1994
; This version is for transparent objects/walls. No input.
MakeCodeTransparent:
		movem.l	ALL,-(sp)
		move.l	lc_F2_WallCodeTrans(pc),a1
		lea		[maxWallHeigth*8](a1),a2	;code table - 2 addresses for top and bottom of screen for transparent walls
		move.l	a2,(a1)+		;fix zero up
		move.l	a2,(a1)+		;fix zero down
		move	#$4e75,(a2)+		;rts
		...
		moveq	#1,d7			;iterate over all heigths (1..31)
.mc_loop32:
		move	d1,d2
		move	d5,d4
		move.l	a2,(a1)+		;up code part addr
		bsr.s	.mc_heigthUnder32Up		; create upper half code
		move	#$4e75,(a2)+		;rts

		move	d1,d2
		move	d5,d4
		move.l	a2,(a1)+		;down code part addr
		bsr.s	.mc_heigthUnder32Down	; create upper half code
		move	#$4e75,(a2)+		;rts

		addq	#1,d7
		cmpi	#32,d7
		bne.s	.mc_loop32
		bra.s	.mc_heigthOver32

.mc_heigthUnder32Up:
		moveq	#1,d6			;n
.mc_l322up: 					; create code within one heigth value	
		moveq	#0,d0
		move	d6,d0
		lsl		#5,d0
		divu	d7,d0
		...
		move	#$1e29,(a2)+		;move.b x(a1),d7
		move	d0,(a2)+
		move	#$6704,(a2)+		;beq *+4
		move	#$1547,(a2)+		;move.b d7,x(a2)
		move	d2,(a2)+			;up
		sub		d3,d2
...
.mc_heigthUnder32Down:
		moveq	#1,d6			;n
.mc_l322down: 					; create code within one heigth value	
		moveq	#0,d0
		move	d6,d0
		lsl		#5,d0
		divu	d7,d0
...
;--------------- Wall heigth bigger than 32 lines...
.mc_heigthOver32:				; d3 - width in pixels
		move	sv_ViewHeigth,d1
		lsr		d1				; limit drawing to half screen up and down
		moveq	#32,d7
.mc_loopMore:
		move.l	lc_F2_LineTab(pc),a3
		moveq	#0,d5			;line lumber
...
.mc_M5:	move	d4,(a3)+
	...
	; --- create code for cell
		move.l	a2,(a1)+		;up code part addr
		move	d2,d0
		neg		d0
		cmpi	#maxWallHeigth-90,d7	; beyond 350-90 = 260 wall heigth just replicate data pointers
		bpl.s	.mc_1
		move	#$43e9,(a2)+		;lea x(a1),a1 - t the start of the line
		move	d0,(a2)+
.mc_1:	move	d6,d0
...
.mc_M8:	cmpi	#maxWallHeigth-90,d7
		bpl.s	.mc_6d
		move	#$4e75,(a2)+		;down rts
.mc_6d:

		cmpi	#maxWallHeigth-90,d7
		bpl.s	.mc_7d
		move.l	-8(a1),a3		;compress code - at full zoom displayed pixels may not change so eliminate duplicates
		move.l	a2,d6
		move.l	a3,d5
		sub.l	d5,d6
		lsr		d6
...
.mc_check1:	
		...
		bne.s	.mc_notequ		;if not the same
		dbf		d6,.mc_check1
		move.l	-8(a1),a2		; roll back pointer
		move.l	-12(a1),-4(a1)	; set pointers to previous ones
		move.l	-16(a1),-8(a1)
		bra.s	.mc_notequ
.mc_7d:
		move.l	-8(a1),a2		; roll back pointer
...
.mc_notequ:	
		...
		move.l	lc_F2_WallCodeTrans(pc),a1		; transparent code tab
		move.l	[(maxWallHeigth-1)*8](a1),d1	; last entry
		move.l	lc_F2_LineTab(pc),a4		; create interpolated Y code tab + Heigth tab
...
mk_DCD2:move.w	(a3)+,(a2)+
		...
		move	d1,d2
		subq	#1,d2
		move	d2,lc_chunkyWidth4_1(a6)	; pixel width/4 -1
		add		d1,d1			;*4
...
mk_FTend: move	d0,(a3)				; on first word remember nr of rows to draw -1
		...
		move	d1,d2
		mulu	lc_screenPixX(a6),d1					; * width in pixels
		...
		lea		(a2),a3
		moveq	#0,d7			;calosc
		move	lc_screenPixX(a6),d2		;wybierz
...
mk_DT2:	move	d3,(a3)+
		...
		lea		(a2,d0.l),a2
		move.l	a2,lc_floorTexture(a6)
		bsr.w	mk_FixFloors
		lea		lc_WallOffsets(pc),a2
		move.l	wall_floor2(a2),d0
		move.l	lc_F1_Walls(pc),a2
		lea		(a2,d0.l),a2
		move.l	a2,lc_ceilingTexture(a6)
		bsr.w	mk_FixFloors
...
;		move	d0,6(a3)
		...
		move.l	lc_F2_RotTable(pc),a2		; correct rotation table (???)
		cmpi.l	#$50100,(a2)
		bne.s	mk_NotFR
		move.l	a2,d2
		moveq	#63,d0
...
mk_fixblood:	move.b	#0,64(a2)
		...
		move.l	lc_F2_Text(pc),a2		;make text offsets
		move.l	a2,d1
		move.l	lc_F2_TextOffsets(pc),a3
.MTO1:	move.b	(a2)+,d0
		cmpi.b	#"@",d0
		bne.s	.MTO1
		move.l	a2,d0
...
mk_FixFloorMod:
		...
		move	lc_variables+lc_screenPixX(pc),d7					;width in pixels (up to 192)
		subq	#1,d7
mk_DCoffsets:	
...
.sv_SWSL2:
		...
		bsr		MakeCodeSolid		;generate raster code for solid walls/objects
		bsr		MakeCodeTransparent	;generate raster code for transparent walls/objects
...
sv_MAP:			equ	BASEC+$2b600		;$8000	-> copied to fast
...
; tables (tables.dat)
sv_Bomba:		equ	BASEC+$4dd34		;$15cc (addr is $34e58 + $18edc) -> copied to fast
sv_RotTable:	equ	BASEC+$4f300		;rot table ($41f2)	-> copied to fast
sv_ChaosTab:	equ	BASEC+$534f2		;$6c0;nie rozbijac bloku! -> copied to fast
...
sc_Text:		equ	BASEC+$7d642		;$1b58 max (7000)	- DO NOT MOVE	-> copied to fast
...
F1_WallCode:		rs.b	$1af00						; Code to draw solid walls/full size objects. $1af00
...
F2_WallCodeTrans:	rs.b	$22100						; Code to draw transparent walls/objects. $220d6
F2_Map:				rs.b	$8000						; copy of map in fast ($9a00 altogether)
F2_LevelData:		rs.b	40							; part of map
F2_EnemyData:		rs.b	4136-40						; part of map
F2_SwitchData:		rs.b	MAP_FULL_SIZE-$8000-4136	; part of map (all adds up to $9a00)
F2_Bomba:			rs.b	$15cc						; bomb gfx
F2_ChaosTab:		rs.b	$6c0						; chaos tab 1
F2_ChaosTac:		rs.b	$800						; chaos tab 2
F2_Numbers:			rs.b	$1ae						; numbers gfx
F2_Fonts:			rs.b	$300						; fonts gfx
F2_Sinus:			rs.b	$280+4						; sinus (256 words) + cosinus (offset 64 words) = 640 (+ small buffer)
F2_TextOffsets:		rs.b	$200						; $100 (to 128 texts)
F2_Text:			rs.b	$2000						; $1b58 max (7000) -> rounded up to $2000
